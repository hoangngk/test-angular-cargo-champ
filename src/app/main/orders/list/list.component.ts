...
import { orderStatusDE, orderStatusEN, orderTypeDE, orderTypeEN, settingsDE, settingsEN } from '../../../../assets/i18n/column-setting-orders.constant';
...
export class ListComponent implements OnInit, OnDestroy {
  ...
  constructor(public datePipe: DatePipe, public orderTitleChange: GetOrderTypeStatusTitlePipe, public translate: TranslateService, private service: SmartTableData, private dialogService: NbDialogService,
    private as: AuthService, private os: OrdersService, public router: Router,
  ) {


    this.os.popupOpenInListPage.subscribe(res => {
      if (res) {
        switch (res.split('@')[0]) {
          case 'edit':
  ...
  ngOnInit(): void {
    ...
    this.as.langChange.subscribe(currLang => {
      if (currLang) {
        ...
        this.orderStatus = currLang === 'en' ? orderStatusEN : orderStatusDE;
    ...
    this.os.getList().subscribe(data => {
      ...
      this.source.load(data.map(el => {
        return {
          ...
          status: this.orderTitleChange.transform(this.orderStatus, el.status),
          offerDate: this.datePipe.transform(el.offerDate , 'yyyy-MM-dd')
        }
      }));
    });
  }
